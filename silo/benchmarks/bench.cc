...
bench_worker::run() {
    ...
    while (running) {
        ...
        if (tBenchServerGetStatus() == 2) { // Checks if warmup finished
            accumulator.push_back({latency, m, sqrt(var)});
            float Z = 1.96; // Z-score for 95-th confidence interval
            float E = 1e4; // Tolerance from true mean, here 100 microseconds
            uint64_t new_sample = Z * Z * var / (E * E);
            uint64_t new_size = std::ceil((new_sample * 0.5 + ops_per_worker * 0.5) / 1000) * 1000;
            ops_per_worker = new_size; // damping updates
            ops_per_worker = ops_per_worker > 2e5 ? 2e5 : ops_per_worker; // Set a maximum window
